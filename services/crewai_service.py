...
def build_tasks(evaluation_result: str) -> List[Task]:
    task1 = Task(
        description=f"""
        아래는 이력서와 자기소개서 평가 결과입니다.  
        이 결과를 기반으로 사용자가 보완해야 할 기술, 자격, 경험 항목을 3~5개 나열해 주세요.

        {evaluation_result}

        반드시 한국어로 리스트 형식으로 출력해 주세요.
        """,
    ...
    task2 = Task(
        ...
        리소스를 초급 → 중급 순으로 정리하고, 2~4주 학습 플랜을 제안하세요.
...
async def run_resume_agent(resume_eval: str, selfintro_eval: str) -> str:
    try:
        evaluation_bundle = f"""[이력서 평가 결과]
            {resume_eval}

            [자기소개서 평가 결과]
            {selfintro_eval}
        """
        tasks = build_tasks(evaluation_bundle)
